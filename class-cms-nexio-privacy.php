...
class CMS_Nexio_Privacy extends WC_Abstract_Privacy {
	/**
	 * Constructor
	 *
	 */
	public function __construct() {
		parent::__construct( __( 'Nexio', 'cms-gateway-nexio' ) );
/*
		$this->add_exporter( 'cms-gateway-nexio-order-data', __( 'WooCommerce Nexio Order Data', 'cms-gateway-nexio' ), array( $this, 'order_data_exporter' ) );

		if ( function_exists( 'wcs_get_subscriptions' ) ) {
			$this->add_exporter( 'cms-gateway-nexio-subscriptions-data', __( 'WooCommerce Nexio Subscriptions Data', 'cms-gateway-nexio' ), array( $this, 'subscriptions_data_exporter' ) );
		}

		$this->add_exporter( 'cms-gateway-nexio-customer-data', __( 'CMS Nexio Customer Data', 'cms-gateway-nexio' ), array( $this, 'customer_data_exporter' ) );

		$this->add_eraser( 'cms-gateway-nexio-customer-data', __( 'CMS Nexio Customer Data', 'cms-gateway-nexio' ), array( $this, 'customer_data_eraser' ) );
		$this->add_eraser( 'cms-gateway-nexio-order-data', __( 'CMS Nexio Data', 'cms-gateway-nexio' ), array( $this, 'order_data_eraser' ) );
	...
	public function account_settings( $settings ) {
		$insert_setting = array(
			array(
				'title'       => __( 'Retain Nexio Data', 'cms-gateway-nexio' ),
				'desc_tip'    => __( 'Retains any Nexio data such as Stripe customer ID, source ID.', 'cms-gateway-nexio' ),
				'id'          => 'woocommerce_gateway_nexio_retention',
				'type'        => 'relative_date_selector',
				'placeholder' => __( 'N/A', 'cms-gateway-nexio' ),
				'default'     => '',
				'autoload'    => false,
			),
		);
	...
	protected function get_stripe_orders( $email_address, $page ) {
		...
		$order_query = array(
			'payment_method' => array( 'nexio', 'nexio_alipay', 'nexio_bancontact', 'nexio_eps', 'nexio_giropay', 'nexio_ideal', 'nexio_multibanco', 'nexio_p24', 'nexio_sepa', 'nexio_sofort' ),
			'limit'          => 10,
			'page'           => $page,
		);
	...
	public function get_privacy_message() {
		...
		return wpautop( sprintf( __( 'By using this extension, you may be storing personal data or sharing data with an external service. <a href="%s" target="_blank">Learn more about how this works, including what you may want to include in your privacy policy.</a>', 'cms-gateway-nexio' ), 'https://docs.woocommerce.com/document/privacy-payments/#cms-gateway-nexio' ) );
	}
		...
		if ( 0 < count( $orders ) ) {
			foreach ( $orders as $order ) {
				$data_to_export[] = array(
					...
					'group_label' => __( 'Orders', 'cms-gateway-nexio' ),
					'item_id'     => 'order-' . $order->get_id(),
					'data'        => array(
						array(
							'name'  => __( 'Nexio payment id', 'cms-gateway-nexio' ),
							'value' => get_post_meta( $order->get_id(), '_stripe_source_id', true ),
						),
						array(
							'name'  => __( 'Nexio customer id', 'cms-gateway-nexio' ),
		...
		if ( 0 < count( $subscriptions ) ) {
			foreach ( $subscriptions as $subscription ) {
				$data_to_export[] = array(
					...
					'group_label' => __( 'Subscriptions', 'cms-gateway-nexio' ),
					'item_id'     => 'subscription-' . $subscription->get_id(),
					'data'        => array(
						array(
							'name'  => __( 'Stripe payment id', 'cms-gateway-nexio' ),
							'value' => get_post_meta( $subscription->get_id(), '_stripe_source_id', true ),
						),
						array(
							'name'  => __( 'Stripe customer id', 'cms-gateway-nexio' ),
	...
	public function customer_data_exporter( $email_address, $page ) {
		...
		if ( $user instanceof WP_User ) {
			...
			$data_to_export[] = array(
				...
				'group_label' => __( 'Customer Data', 'cms-gateway-nexio' ),
				'item_id'     => 'user',
				'data'        => array(
					array(
						'name'  => __( 'Stripe payment id', 'cms-gateway-nexio' ),
	...
	public function customer_data_eraser( $email_address, $page ) {
		...
		if ( ! empty( $stripe_customer_id ) || ! empty( $stripe_source_id ) ) {
			...
			$messages[] = __( 'Stripe User Data Erased.', 'cms-gateway-nexio' );
		}
	...
	protected function maybe_handle_subscription( $order ) {
		...
		if ( ! $this->is_retention_expired( $order->get_date_created()->getTimestamp() ) ) {
			...
			return array( false, true, array( sprintf( __( 'Order ID %d is less than set retention days. Personal data retained. (Stripe)', 'cms-gateway-nexio' ), $order->get_id() ) ) );
		}

		if ( $subscription->has_status( apply_filters( 'wc_stripe_privacy_eraser_subs_statuses', array( 'on-hold', 'active' ) ) ) ) {
			/* translators: %d Order ID */
			return array( false, true, array( sprintf( __( 'Order ID %d contains an active Subscription. Personal data retained. (Stripe)', 'cms-gateway-nexio' ), $order->get_id() ) ) );
		}
		...
		return array( true, false, array( __( 'Stripe Subscription Data Erased.', 'cms-gateway-nexio' ) ) );
	}
	...
	protected function maybe_handle_order( $order ) {
		...
		return array( true, false, array( __( 'Stripe personal data erased.', 'cms-gateway-nexio' ) ) );
	}
	...
	public function is_retention_expired( $created_date ) {
		$retention  = wc_parse_relative_date_option( get_option( 'woocommerce_gateway_nexio_retention' ) );
		$is_expired = false;
